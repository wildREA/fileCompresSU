...
// Types for better type safety at the global level
...
// Use a self-executing function to avoid global exports issues
const compression = (function() {
  ...
  function setupDropzone(
    ...
    fileInputElement.addEventListener('change', (event) => {
      ...
      if (target.files && target.files.length > 0) {
        const currentFileIds = new Set(selectedFiles.map(f => f.id || generateFileId(f)));
        ...
        const newFiles = fileInfos.filter(file => !currentFileIds.has(file.id!));
   ...
   */
  function compressFiles(filePaths: string[]): Promise<CompressedFile[]> {
  ...
  function updateDownloadSection(
    ...
    compressedFiles.forEach(file => {
      ...
      downloadButton.className = 'bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm ml-4 transition-colors duration-150';
